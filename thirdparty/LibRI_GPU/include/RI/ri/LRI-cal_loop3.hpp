	...
	const double fac_add_Ds)
{
	...
	//* 计时
	constexpr int timing_number = 20;
	struct timeval start[timing_number], end[timing_number];
	std::vector<double> elapse(timing_number, 0.0);
	double little=0;

	gettimeofday(&start[0], NULL); // 总计时
			...
			switch(label)
			{
			  ...
			  // Aab_Aab::a01b01_a01b01
				...
				{	
					...
					// gettimeofday(&start[5], NULL); // Aa2 大循环用时
					for(const TAC &Aa2 : list_Aa2)
					{
						if(this->filter_atom->filter_for1(label,Aa2))	continue;
						std::map<TAC,Tensor<Tdata>> Ds_result_fixed;

						// gettimeofday(&start[6], NULL); // Ab01 大循环用时
						#pragma omp for schedule(dynamic) nowait
						for(std::size_t ib01=0; ib01<list_Ab01.size(); ++ib01)
						{
							// gettimeofday(&start[9], NULL); // filter_for2 用时
							const TAC &Ab01 = list_Ab01[ib01];

							if(this->filter_atom->filter_for2(label,Aa2,Ab01))	
								continue;
							// D_mul = D_a * D_a0b0 * D_a1b1
							Tensor<Tdata> D_mul;
							// gettimeofday(&end[9], NULL);
							// elapse[9] += get_time(&start[9], &end[9]);

							// gettimeofday(&start[7], NULL); // Aa01 小循环
							for(const TA &Aa01 : list_Aa01)
							{
								// std::cout << "Aa2: " << Aa2.first << "\t"
								// << "Ab01: " << Ab01.first << "\t"
								// << "Aa01: " << Aa01 << std::endl;
								gettimeofday(&start[3], NULL); // 检查 C,V,D 用时
								...
								elapse[3] += get_time(&start[3], &end[3]);

								gettimeofday(&start[1], NULL); // X 的计算时长
								// a1a2b0 = a0a1a2 * a0b0
								...
								elapse[1] += get_time(&start[1], &end[1]);
								// std::cout << "完全通过，执行了乘法" << std::endl;
							}
							// gettimeofday(&end[7], NULL);
							// const double duration = get_time(&start[7], &end[7]);
							// elapse[7] += duration;
							...
							// {
							// 	std::ofstream ofs("X_"+std::to_string(Aa2.first)+"_"+std::to_string(Ab01.first)+"_orig.txt");
							// 	ofs<<D_mul<<std::endl;
							// }
							// {
							// 	std::ofstream ofs("X_"+std::to_string(Aa2.first)+"_"+std::to_string(Ab01.first)+".dat", std::ofstream::binary);
							// 	cereal::BinaryOutputArchive ar(ofs);
							// 	ar(D_mul);
							// }

							// D_result = D_mul * D_b
							// gettimeofday(&start[8], NULL); // Ab2 小循环
							for(const TAC &Ab2 : list_Ab2)
							{
								gettimeofday(&start[4], NULL); // 检查 Cb 用时
								...
								elapse[4] += get_time(&start[4], &end[4]);
								
								gettimeofday(&start[2], NULL); // H 的计算时长
								// a2b2 = a2b0b1 * b0b1b2
								...
								elapse[2] += get_time(&start[2], &end[2]);
							}
							// gettimeofday(&end[8], NULL);
							// elapse[8] += get_time(&start[8], &end[8]);
						} // end for Ab01
						// gettimeofday(&end[6], NULL);
						// elapse[6] += get_time(&start[6], &end[6]);
					...
					// elapse[5] += get_time(&start[5], &end[5]);
	...
	elapse[0] = get_time(&start[0], &end[0]);
	std::cout << "-----------------------------------" << std::endl;
	std::cout << "单位：毫秒" << std::endl;
	std::cout << "总时长\t"
				<< "X计算\t"
				<< "H计算\t"
				<< "检查CVD\t"
	...
	std::cout << "零零总总的计时：" << little << std::endl;
	...
	//	{
	//		std::ofstream ofs("H_"+std::to_string(Ds_A.first)+"_"+std::to_string(Ds_B.first.first)+"_orig.txt");
