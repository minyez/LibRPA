...
void task_test()
{
    ...
    // Debug: read k-space Fock matrices from FHI-aims output, a check for GaAs
    if (LIBRPA::envs::size_global > 1)
    {
        throw logic_error("only support single MPI task");
    }

    Profiler::start("debug");
    int n_basis;
    bool is_real;
    std::vector<Matz> exx_ij_k;
    std::vector<double> mat;
    for (int ik = 0; ik < kfrac_list.size(); ik++)
    {
        stringstream s;
        s << "hf_exchange_spin_01_kpt_" << std::setfill('0') << std::setw(6) << ik + 1 << ".csc";
        read_elsi_csc(s.str(), true, mat, n_basis, is_real);
        Matz ex(n_basis, n_basis, reinterpret_cast<complex<double>*>(mat.data()), MAJOR::ROW);
        exx_ij_k.push_back(ex);
    }
    // replace the real-space exx matrix computed by LibRPA
    for (auto &s_exx_RIJ: exx.exx)
    {
        for (auto &R_exxIJ: s_exx_RIJ.second)
        {
            const auto &R = R_exxIJ.first;
            for (auto &I_exxJ: R_exxIJ.second)
            {
                const auto &I = I_exxJ.first;
                const auto n_I = LIBRPA::atomic_basis_wfc.get_atom_nb(I);
                for (auto &J_exx: I_exxJ.second)
                {
                    const auto &J = J_exx.first;
                    const auto n_J = LIBRPA::atomic_basis_wfc.get_atom_nb(J);
                    auto &mat = J_exx.second;
                    mat.zero_out();
                    // Inverse FT. Not optimized
                    for (int ik = 0; ik < kfrac_list.size(); ik++)
                    {
                        const auto &kfrac = kfrac_list[ik];
                        const auto ang = - kfrac * R * TWO_PI;
                        const auto phase = std::complex<double>{std::cos(ang), std::sin(ang)};
                        // slice the atom-pair block
                        Matz mat_ij_k(n_I, n_J, MAJOR::ROW);
                        for (int i = 0; i < n_I; i++)
                        {
                            for (int j = 0; j < n_I; j++)
                            {
                                const int i_g = LIBRPA::atomic_basis_wfc.get_global_index(I, i);
                                const int j_g = LIBRPA::atomic_basis_wfc.get_global_index(J, j);
                                mat_ij_k(i, j) = exx_ij_k[ik](i_g, j_g);
                            }
                        }
                        // cout << (mat_ift.major() == mat.major()) << " " << mat_ift.size() << " " << mat.size() << endl;
                        mat -= (mat_ij_k * phase).get_real();
                    }
                    // Normalize
                    mat /= double(kfrac_list.size());
                }
            }
        }
    }
    Profiler::cease("debug");
