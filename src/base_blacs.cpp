...
void BLACS_CTXT_handler::set_grid(const int &nprows_in, const int &npcols_in, CTXT_LAYOUT layout_in)
{
    // if the grid has been set, exit it first
    if (pgrid_set_) exit();
    if (nprocs != nprows_in * npcols_in) throw std::invalid_argument("nprocs != nprows * npcols");
    layout = layout_in;
...
void BLACS_CTXT_handler::set_square_grid(bool more_rows, CTXT_LAYOUT layout_in)
{
    ...
    more_rows ? nprows = nprocs / (npcols = nroc) : npcols = nprocs / (nprows = nroc);
    set_grid(nprows, npcols, layout_in);
}

void BLACS_CTXT_handler::set_horizontal_grid() { set_grid(1, nprocs, CTXT_LAYOUT::R); }

void BLACS_CTXT_handler::set_vertical_grid() { set_grid(nprocs, 1, CTXT_LAYOUT::C); }
...
{
    std::string info;
    info = std::string("BLACS_CTXT_handler: ") + "ICTXT " + std::to_string(ictxt) + " " + "PSIZE " +
           std::to_string(nprocs) + " " + "PID " + std::to_string(myid) + " " + "PGRID (" +
           std::to_string(nprows) + "," + std::to_string(npcols) + ") " + "PCOOD (" +
           std::to_string(myprow) + "," + std::to_string(mypcol) + ")";
    return info;
}
...
void BLACS_CTXT_handler::barrier(CTXT_SCOPE scope) const { CTXT_barrier(ictxt, scope); }

void Array_Desc::set_blacs_params_(int ictxt, int nprocs, int myid, int nprows, int myprow,
                                   int npcols, int mypcol)
{
                          ...
                          const int &irsrc, const int &icsrc)
{
    ...
    if (info)
    {
        LIBRPA::utils::lib_printf(
            ...
            myprow_, mypcol_, m, n, mb, nb, irsrc, icsrc, ictxt_, m_local_);
    }
    // else
    //     LIBRPA::utils::lib_printf("SUCCE DESCINIT! PROC %d (%d,%d) PARAMS: DESC %d %d %d %d %d %d
      ...
      initialized_(false)
{
    ...
    set_blacs_params_(blacs_h.ictxt, blacs_h.nprocs, blacs_h.myid, blacs_h.nprows, blacs_h.myprow,
      ...
      initialized_(false)
{
    ...
    set_blacs_params_(ictxt, nprocs, myid, nprows, myprow, npcols, mypcol);
}

int Array_Desc::init(const int &m, const int &n, const int &mb, const int &nb, const int &irsrc,
                     const int &icsrc)
{
    return set_desc_(m, n, mb, nb, irsrc, icsrc);
}

int Array_Desc::init_1b1p(const int &m, const int &n, const int &irsrc, const int &icsrc)
{
    int mb = 1, nb = 1;
...
int Array_Desc::init_square_blk(const int &m, const int &n, const int &irsrc, const int &icsrc)
{
...
{
    std::string info;
    info = std::string("ArrayDesc: ") + "ICTXT " + std::to_string(ictxt_) + " " + "ID " +
           std::to_string(myid_) + " " + "PCOOR (" + std::to_string(myprow_) + "," +
           std::to_string(mypcol_) + ") " + "GSIZE (" + std::to_string(m_) + "," +
           std::to_string(n_) + ") " + "LSIZE (" + std::to_string(m_local_) + "," +
           std::to_string(n_local_) + ") " + "DUMMY? " + std::string(empty_local_mat_ ? "T" : "F");
    return info;
}
...
{
    char s[100];
    sprintf(s, "DESC %d %d %d %d %d %d %d %d %d", desc[0], desc[1], desc[2], desc[3], desc[4],
...
void Array_Desc::barrier(CTXT_SCOPE scope) { CTXT_barrier(ictxt_, scope); }
