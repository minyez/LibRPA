...
int atom_mu_glo2loc(const int &glo_index, int &mu_index)
{
    ...
    {
        if ((mu_index = glo_index - atom_mu_part_range[I]) < atom_mu[I]) return I;
    }
...
matrix reshape_Cs(size_t n1, size_t n2, size_t n3,
                  const shared_ptr<matrix> &Csmat)  //(n1*n2,n3) -> (n2,n1*n3)
{
...
matrix reshape_mat(const size_t n1, const size_t n2, const size_t n3,
                   const matrix &mat)  //(n1,n2*n3) -> (n2,n1*n3)
{
    const auto length = sizeof(double) * n3;
    const auto n13 = n1 * n3;
...
matrix reshape_mat_21(const size_t n1, const size_t n2, const size_t n3,
                      const matrix &mat)  //(n1,n2*n3) -> (n1*n2,n3)
{
...
void init_N_all_mu()
{
    ...
    if (Params::DFT_software == "ABACUS")
    {
        ...
        MPI_Allreduce(loc_nw.data(), glo_nw.data(), natom, MPI_UNSIGNED_LONG_LONG, MPI_MAX,
                      mpi_comm_global);
        MPI_Allreduce(loc_mu.data(), glo_mu.data(), natom, MPI_UNSIGNED_LONG_LONG, MPI_MAX,
                      mpi_comm_global);
    ...
    for (int I = 0; I != natom; I++)
        for (int J = 0; J != natom; J++)
        {
            ...
            for (const auto &Rp : Cs[I][J])
            {
                auto R = Rp.first;
                // if(I==0 && J==0)
                //     LIBRPA::utils::lib_printf(" send R  Cs  myid : %d   I: %d, J: %d, R:(%d, %d,
            ...
            MPI_Allgatherv(send_R_data.data(), loc_count, MPI_INT, all_R_data.data(),
                           all_R_counts.data(), displs.data(), MPI_INT, mpi_comm_global);

            int nR = total_R_count / 3;
            for (int iR = 0; iR != nR; iR++)
            {
                Vector3_Order<int> Rv(all_R_data[iR * 3], all_R_data[iR * 3 + 1],
                                      all_R_data[iR * 3 + 2]);
                // if(I==0 && J==0)
                //     LIBRPA::utils::lib_printf(" Rv  Cs  myid : %d   I: %d, J: %d, R:(%d, %d,
                ...
                if (!Cs[I][J].count(Rv)) Cs[I][J][Rv] = cs_ptr;
            }
        }
}
    ...
    for (auto &vf_p : Vq_glo)
    {
        ...
        for (int I = 0; I != atom_mu.size(); I++)
            for (int J = 0; J != atom_mu.size(); J++)
            {
                ...
                // cout << "I J: " << I << "  " << J << "   mu,nu: " << atom_mu[I] << "  " <<
                // atom_mu[J] << endl;
                for (int i_mu = 0; i_mu != atom_mu[I]; i_mu++)
                {
                    for (int i_nu = 0; i_nu != atom_mu[J]; i_nu++)
                    {
                        //(*vq_ptr)(i_mu, i_nu) = vf_p.second(atom_mu_loc2glo(J, i_nu),
                        // atom_mu_loc2glo(I, i_mu)); ////for aims
                        (*vq_ptr)(i_mu, i_nu) = vf_p.second(
                            atom_mu_loc2glo(I, i_mu), atom_mu_loc2glo(J, i_nu));  // for abacus
                    }
                }
