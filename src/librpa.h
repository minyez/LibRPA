...
extern "C"
{
    ...
    struct LibRPAParams
    {
        ...
        //! Threshold of real-space LRI triple coefficients to compute GW correlation self-energy
        //! using LibRI
        double libri_gw_threshold_C;

        //! Threshold of real-space Green's function to compute GW correlation self-energy using
        //! LibRI
        double libri_gw_threshold_G;

        //! Threshold of correlation screened Coulomb matrix to compute GW correlation self-energy
        //! using LibRI
     ...
     * @param[in] output_filename    Name of file for redirected output. Only used when
     * redirect_stdout is true
     */
     ...
     * @param[in] mf        meanfield
     *
     * The array `wg` and `ekb` must be stored in C-style row-major order.
     */
    void set_wg_ekb_efermi(int nspins, int nkpts, int nstates, double* wg, double* ekb,
                           double efermi, MeanField& mf);
     ...
     * @param[in] G_mat      pointer to array of reciprocal-space lattice vectors, in inverse Bohr
     * unit
     */
     ...
     * @param[in] kvecs    Coordinates of k-mesh vectors, in inverse Bohr unit, dimension:
     * [nk1*nk2*nk3][3]
     *
     ...
     */
    void set_ibz2bz_index_and_weight(const int nk_irk, const int* ibz2bz_index,
                                     const double* wk_irk);

    /*!
     * @brief Insert the atom index and set the local RI triple coefficients
     *
     * @param[in] I                    Index of atom, where one basis and the auxiliary basis
     * reside.
     ...
     * @param[in] Cs_in                Local RI triple coefficients, dimension:
     * [nbasis_i][nbasis_j][naux_mu]
     ...
     * @param[in] Vq_imag_in    Imaginary part of the Coulomb matrix block, dimension:
     * [naux_mu][naux_nu]
     *
     ...
     * @param[in] naux_mu       Number of auxliary basis functions centered at atom I
     * @param[in] naux_nu       Number of auxliary basis functions centered at atom J
     * @param[in] Vq_real_in    Real part of the truncated Coulomb matrix block, dimension:
     * [naux_mu][naux_nu]
     * @param[in] Vq_imag_in    Imaginary part of the truncated Coulomb matrix block, dimension:
     * [naux_mu][naux_nu]
     *
     * The array `Vq_real_in` and `Vq_imag_in` must be stored in C-style row-major order.
     */
     ...
     * @param[in] Vq_real_in    Real part of the Coulomb matrix block, dimension:
     * [mu_begin-mu_end+1][nu_begin-nu_end+1]
     * @param[in] Vq_imag_in    Imaginary part of the Coulomb matrix block, dimension:
     * [mu_begin-mu_end+1][nu_begin-nu_end+1]
     *
     * The array `Vq_real_in` and `Vq_imag_in` must be stored in C-style row-major order.
     */
    void set_aux_bare_coulomb_k_2D_block(int ik, int max_naux, int mu_begin, int mu_end,
     ...
     * The array `Vq_real_in` and `Vq_imag_in` must be stored in C-style row-major order.
     */
    void set_aux_cut_coulomb_k_2D_block(int ik, int max_naux, int mu_begin, int mu_end,
                                        int nu_begin, int nu_end, double* Vq_real_in,
                                        double* Vq_imag_in);
     ...
     *                               `n_spins` * `n_kpoints_task` * (`i_state_high` -
     * `i_state_low`). When `n_kpoints_task` < 0, the length should be at least `n_spins` *
     * `n_kpoints` * (`i_state_high` - `i_state_low`).
     */
    void compute_exx_orbital_energy(int i_state_low, int i_state_high, int n_kpoints_task,
                                    const int* i_kpoints_task, double* exx);

    /*
     * @brief Compute the real-space, imaginary-frequency screened Coulomb matrix in auxiliary basis
     * representation
     *
     * @warning
     * Not implemented yet.
    ...
    /*
     * @brief Compute the reciprocal-space, imaginary-frequency screened Coulomb matrix in auxiliary
     * basis representation
     *
     * @warning
     * Not implemented yet.
     */
    void compute_screened_coulomb_abf_recpspace_imagfreq(int n_qpoints_task,
                                                         const int* i_qpoints_task,
                                                         int n_freqs_task, const int* i_freqs_task,
                                                         double* screened_coul);
