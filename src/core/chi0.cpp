        ...
        if (chi0_q_in.count(q) > 0)
        {
            for (auto &Ip : chi0_q_in[q])
            {
                ...
                for (auto &Jm : Ip.second)
                {
                    ...
                    for (int ir = 0; ir < abf_small[I]; ir++)
                    {
                        for (int ic = 0; ic < abf_small[J]; ic++)
                        {
    ...
    for (size_t it = 0; it != tfg.size(); it++)
    {
        ...
        for (auto isp = 0; isp < this->mf.get_n_spins(); isp++)
        {
            ...
            else
            {
                ...
                chi_libri_ft_ct<Tdata>(isp, mf.get_n_spins(), it, tfg, atbasis_abf, pbc.latvec, chi0s_IJR, qlist,
                                       atpairs_ABF, chi0_q);
                ...
                for (auto &IJqc : Wc.at(freq))
                {
                    ...
                    for (auto &Jqc : IJqc.second)
                    {
                        ...
                        for (auto &qc : Jqc.second)
                        {
                            ...
                            for (int ir = 0; ir < abf_unfold[I]; ir++)
                            {
                                for (int ic = 0; ic < abf_unfold[J]; ic++)
                                {
